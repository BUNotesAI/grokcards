...
describe("buildEdges", () => {
  ...
  it("混合所有类型 — 整体场景", () => {
    ...
    const set = new Set(["c1", "c2", "c3", "n1", "a1"]);
    const edges = buildEdges(cards, notes, aliases, set);
    // 1 link_to (c1→c2) + 1 note_link (n1→c1) + 1 alias_link (a1→c3) = 3
    // related (c1→c3) 不产生 edge
    expect(edges).toHaveLength(3);
  });
});
